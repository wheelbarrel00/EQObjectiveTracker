...
L["Hide tracker when no quests are showing"] = true
L["Hides the whole tracker while you have no quest or campaign rows, and brings it back the moment you accept one. Nothing else keeps it on screen by itself, including world quests, achievements, the zone progress bar, and the delve or scenario panel."] = true
...
L["Reset"] = true
L["Cancel"] = true
L["Restores every setting on every tab to its default and reloads the interface. This also clears the Options Window Scale, which every character shares, and this character's collapsed sections and pinned entries."] = true
L["Profiles"] = true
...
L["Play a sound when you accept a quest"] = true
L["Off by default. It has its own sound below, so accepting and completing can be told apart."] = true
L["Quest Accepted Sound"] = true
L["Which sound plays when you accept a quest. World quests and bonus objectives are left silent, since walking into one accepts it."] = true
L["Play a sound when you turn a quest in"] = true
...
L["Event Title Text Size"] = true
L["Grows or shrinks the event title above the scenario / delve banner, the line naming the scenario itself. This value is added to the Font Size above, so 4 is the default and keeps the title sizing like a section heading. A long title wraps rather than trailing off, so large values make the panel taller."] = true
L["Event Title Color"] = true
L["Color of the event title above the scenario / delve banner. It matches the Section Header Color by default but is set separately, because the scenario panel is not one of the tracker's sections."] = true
L["Scroll Bar"] = true
